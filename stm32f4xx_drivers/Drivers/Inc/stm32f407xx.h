...
/*
 * ARM Cortex Mx Processor Interrupt Set-enable Registers (only 0-3 required)
 */
...
/*
 * ARM Cortex Mx Processor Interrupt Clear-enable Registers
 */
...
*  Each priority register looks something like this. For example writing a priority of 5 (1010) ==> 1010 0000 in the register
* ┌────────┬───────┐
* │ USED   │unused │
* │ 4 bits │4 bits │
* └────────┴───────┘
 ...
 */
 ...
 /*
 * peripheral register definition structure for GPIOx (stuff you can configure/read on a GPIO port)
 */
typedef struct
{
	...
	__vo uint32_t BSRR;
	__vo uint32_t LCKR;
	__vo uint32_t AFR[2];					              /*!< AFR[0] : GPIO alt func low register (pin 0-7), AF[1] : GPIO alt func low register (pin 8-15) 		Address offset: 0x20 and 0x24 */
}GPIO_RegDef_t;


/*
 * peripheral register definition structure for RCC (reset clock control, used to enable/disable clock signals sent to different peripherals)
 */
typedef struct
{
 ...
 * NOTE: This is actually chaining ternary operators together ==> condition ? value : <another_ternary>, and the backslash just means this define continues to the next line
 */
